...
# youtube
  ...
  curl --no-progress-meter "https://img.youtube.com/vi/$url/0.jpg" | chafa \
    ...
    --scale max
# Discord 
elif [[ "$item" =~ ^https:\/\/(cdn|media).discordapp.(net|com)\/(attachments|external|avatars)\/ ]]; then
  item_full="$(echo -n "$item_raw" | cliphist decode)"
  curl --no-progress-meter "$item_full" --output ~/.cache/discord

  filetype="$(file ~/.cache/discord | awk '{print $4}')"
  if [[ "$filetype" == "MP4" ]]; then
    ffmpeg -y -hide_banner -loglevel error -i ~/.cache/discord -ss 00:00:01.000 -vframes 1 ~/.cache/discord.png
    cp ~/.cache/discord.png ~/.cache/discord
  fi
  chafa ~/.cache/discord \
    -f sixels \
    --align center \
